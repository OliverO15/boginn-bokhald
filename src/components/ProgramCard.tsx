...
import {
  calculateMonthlyInstructorWages,
  calculateSeasonalInstructorWages,
} from "@/lib/instructor-calculations";
...
export function ProgramCard({
  ...
  const getQuantity = (pricingOptionId: string): number => {
    ...
    if (currentRegistration) {
      const entry = currentRegistration.entries.find(
        (e) => e.pricingOption.id === pricingOptionId
      );
    ...
    if (isMonthly && previousMonthRegistration) {
      const entry = previousMonthRegistration.entries.find(
        (e) => e.pricingOption.id === pricingOptionId
      );
      return entry?.quantity || 0;
    }
  ...
  if (!isMonthly && currentSeason) {
    // Seasonal programs - show seasonal numbers divided by 4 for monthly view
    const seasonalRegistrations = program.registrations.filter(r => 
      r.seasonId === currentSeason.id && r.month === null
    );
    
    seasonalRegistrations.forEach(registration => {
    ...
    // Monthly programs - use local quantity logic
    program.pricingOptions.forEach((option) => {
      ...
      if (quantity > 0) {
        ...
        const venueCost =
          revenue *
          ((program.venueSplitPercentNew || program.venueSplitPercent) / 100);
  ...
  // Calculate instructor wages for this program
  const currentYear = new Date().getFullYear();
  let instructorWages = 0;

  if (program.instructors && program.instructors.length > 0) {
    if (isMonthly) {
      // Monthly program - calculate for current month
      instructorWages = calculateMonthlyInstructorWages(
        program,
        currentMonth,
        currentYear
      );
      ...
      instructorWages = seasonalWage / 4;
    }
  }

  const totalCosts = totalVenueCosts + instructorWages;
  ...
  const applyChanges = async () => {
    ...
    try {
      ...
      const updates = Array.from(localQuantities.entries()).map(
        ([pricingOptionId, quantity]) =>
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {isMonthly
              ? `${getCurrentMonthName(currentMonth)} ${new Date().getFullYear()}`
              : 'Season Program'}
          </p>
        </div>
      ...
      <div className="space-y-3 mb-4">
        {program.pricingOptions.map((option) => {
          ...
          const hasLocalChanges =
            localQuantities.has(option.id) &&
            (!currentRegistration ||
              !currentRegistration.entries.find(
                (e) => e.pricingOption.id === option.id
              ) ||
              currentQuantity !==
                (currentRegistration.entries.find(
                  (e) => e.pricingOption.id === option.id
          ...
          return (
            <div key={option.id} className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() =>
                    updateLocalQuantity(option.id, currentQuantity - 1)
                  }
                ...
                <span
                  className={`w-8 text-center font-mono text-sm ${
                    hasLocalChanges ? "text-orange-600 font-bold" : ""
                  }`}
                >
                  {currentQuantity}
                </span>
      ...
      <div className="border-t border-gray-200 pt-4 space-y-2">
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="font-medium">
            {totalRevenue.toLocaleString()} ISK
          </span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="flex items-center">🏢 Venue Costs:</span>
          <span className="font-medium">
            {totalVenueCosts.toLocaleString()} ISK
          </span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="flex items-center">👨‍🏫 Instructor Wages:</span>
          <span className="font-medium">
            {Math.round(instructorWages).toLocaleString()} ISK
          </span>
        </div>
        <div className="flex justify-between items-center text-sm font-semibold">
          ...
          >
            {Math.round(totalProfit).toLocaleString()} ISK
          </span>
        </div>

...
export default function NewFlexibleProgramPage({ params }: PageProps) {
  ...
  const fetchAvailableProgramTypes = async () => {
    try {
      const response = await fetch(
        `/api/years/${yearId}/available-program-types`
      );
      if (response.ok) {
  ...
  const selectedProgramType = programTypes.find(
    (pt) => pt.id === selectedProgramTypeId
  );
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Create a program with fully customizable pricing options and
            settings
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                ...
                {selectedProgramType && (
                  <p className="mt-1 text-sm text-gray-500">
                    Default: {selectedProgramType.sessionHours}h sessions,{" "}
                    {selectedProgramType.isMonthly ? "monthly" : "seasonal"}
                  </p>
                )}
              </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <p className="mt-4 text-sm text-gray-500">
              You can reorder pricing options using the ▲▼ buttons. Inactive
              options will not be shown to users but can be reactivated later.
            </p>
          </div>
